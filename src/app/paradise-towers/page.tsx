        ...
        <div className="relative z-20 container mx-auto px-6 lg:px-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="max-w-3xl">
            ...
            <p className="text-slate-200 text-lg md:text-xl font-light">
              Diseñadas para elevar tu estilo de vida. Un complejo residencial que combina arquitectura vanguardista con una ubicación envidiable sobre la majestuosa elevación de Larimar City.
            </p>
          </div>
          <div className="shrink-0 flex flex-col gap-3 w-full md:w-auto">
            <button className="w-full md:w-auto px-8 py-4 bg-secondary text-white hover:bg-white hover:text-primary transition-colors flex items-center justify-center font-medium rounded-sm">
              Descargar Catálogo <Download className="ml-2 w-4 h-4" />
            </button>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="grid lg:grid-cols-3 gap-16">
            <div className="lg:col-span-2">
              <h2 className="text-3xl font-playfair font-bold text-primary mb-6">Un Oasis de Bienestar</h2>
              <p className="text-slate-600 mb-6 leading-relaxed">
                Paradise Towers no es solo un conjunto de edificios; es una declaración de intenciones. Cada torre ha sido posicionada para maximizar la entrada de luz natural y ofrecer las mejores puestas de sol de Punta Cana. Con áreas sociales integradas y senderos verdes, es el lugar perfecto para quienes buscan equilibrio.
              </p>

              <h3 className="text-xl font-bold text-primary mt-10 mb-4">Amenidades Exclusivas</h3>
            ...
            <div className="bg-stone-50 p-8 rounded-md border border-slate-100">
              <h3 className="text-xl font-bold text-primary mb-6 border-b border-slate-200 pb-4">Documentación Inversor</h3>
              <div className="space-y-4">
                <Link href="#" className="flex items-center p-4 bg-white rounded-sm border border-slate-200 hover:border-secondary transition-all group">
                  ...
                  <div>
                    <p className="font-semibold text-primary">BrochureParadise.pdf</p>
                ...
                <Link href="#" className="flex items-center p-4 bg-white rounded-sm border border-slate-200 hover:border-secondary transition-all group">
                  ...
                  <div>
                    <p className="font-semibold text-primary">Tipologías de Apto.</p>
                    <p className="text-xs text-slate-500">Imágenes • 12 MB</p>
                  </div>
                </Link>
              </div>
            </div>
      ...
      <section className="py-20 px-6 lg:px-12 bg-primary text-white">
        <div className="container mx-auto max-w-5xl">
          <div className="mb-12 text-center">
            <h2 className="text-3xl font-playfair font-bold text-white mb-4">Inventario de Unidades</h2>
            <p className="text-slate-300 font-light">Unidades disponibles en preventa. Entrega estimada: 2026.</p>
          </div>
                ...
                <tbody className="divide-y divide-white/10 text-slate-200 text-sm">
                  {[
                    ...
                    <tr key={i} className="hover:bg-white/5 transition-colors text-white/80">
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-1 text-xs rounded-sm font-medium ${unit.status === 'Disponible' ? 'bg-green-500/20 text-green-300' :
                            unit.status === 'Vendida' ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'
                          }`}>
                          {unit.status}
                        </span>
                      </td>
