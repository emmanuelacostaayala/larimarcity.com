            ...
            <div className="container mx-auto px-4 py-24 max-w-6xl">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
                    ...
                    <div className="lg:col-span-5">
                        ...
                        >
                            <h1 className="font-playfair text-5xl md:text-6xl mb-6">Agenda una <span className="text-gold italic">Visita</span></h1>
                    ...
                    <div className="lg:col-span-7 bg-[#121c30] border border-white/10 p-8 md:p-12">
                        {step === 1 ? (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                <h3 className="font-playfair text-3xl mb-8 flex items-center gap-3">
                                    <Calendar className="text-gold" /> Selecciona el Horario
                                </h3>
                                    ...
                                    {slots.map((slot) => (
                                        <button
                                            ...
                                            className={`p-4 border text-sm font-semibold transition-all ${selectedSlot === slot
                                                    ? "bg-gold text-[#0a101f] border-gold"
                                ...
                                <button
                                    ...
                                    onClick={() => setStep(2)}
                                    className="w-full py-5 bg-white text-[#0a101f] disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gold transition-colors uppercase tracking-widest text-xs font-bold flex items-center justify-center gap-2"
                                >
                                ...
                                <form className="flex flex-col gap-6" onSubmit={(e) => { e.preventDefault(); setStep(3); }}>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input required type="text" className="w-full bg-transparent border-b border-white/20 focus:border-gold py-2 text-white outline-none" />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] uppercase tracking-widest text-white/40 mb-2">Email</label>
                                            <input required type="email" className="w-full bg-transparent border-b border-white/20 focus:border-gold py-2 text-white outline-none" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="block text-[10px] uppercase tracking-widest text-white/40 mb-2">Sesión para el día {selectedSlot}</label>
                                        <textarea rows={3} placeholder="¿Alguna pregunta específica que quieras tratar?" className="w-full bg-transparent border-b border-white/20 focus:border-gold py-2 text-white outline-none resize-none" />
                                    </div>
                                    <div className="flex gap-4">
                                        ...
                                        <button type="submit" className="flex-1 py-4 bg-gold text-[#0a101f] hover:bg-white transition-colors text-xs uppercase tracking-widest font-bold">Confirmar Cita</button>
                                    </div>
                                </form>
                            </motion.div>
                        )}
                        ...
                        {step === 3 && (
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                                <div className="w-20 h-20 bg-gold/10 text-gold mx-auto rounded-full flex items-center justify-center mb-6">
                                ...
                                <button onClick={() => setStep(1)} className="text-gold uppercase tracking-[0.2em] text-xs font-bold hover:text-white transition-colors">Agendar otra</button>
                            </motion.div>
                        )}
                    </div>
