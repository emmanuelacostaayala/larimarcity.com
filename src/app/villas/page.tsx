            ...
            <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
                ...
                <div className="container mx-auto px-4 relative z-10 text-center">
                    <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="font-playfair text-6xl md:text-8xl mb-4 text-white">
                        Villas & <span className="text-gold italic">Townhouses</span>
                    </motion.h1>
                    <p className="text-white/40 uppercase tracking-[0.4em] text-sm">Arquitectura que define tu estilo de vida</p>
                </div>
            </section>
                    ...
                    {options.map((opt, idx) => (
                        ...
                        >
                            ...
                            <div className="absolute bottom-0 left-0 p-12 w-full">
                                <h3 className="font-playfair text-4xl mb-4 text-white group-hover:text-gold transition-colors">{opt.title}</h3>
                                <p className="text-white/60 mb-8 max-w-md font-light">
                                    {opt.desc}
                                </p>
                                <Link
                                    href={opt.link}
                                    className="inline-flex items-center gap-2 text-gold text-xs uppercase tracking-widest font-bold border-b border-gold/30 pb-2 hover:border-gold transition-all"
                                >
                                    Ver Detalles Proyecto <ArrowRight className="w-4 h-4" />
                                </Link>
                            </div>
                        </motion.div>
                    ))}
