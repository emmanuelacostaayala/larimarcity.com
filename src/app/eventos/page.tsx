      ...
      <section className="py-24 container mx-auto px-4 max-w-7xl">
        <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            <span className="text-gold uppercase tracking-[0.3em] text-xs font-bold mb-4 block">Experiencias Larimar</span>
            <h1 className="font-playfair text-6xl md:text-8xl">Eventos & <br /><span className="text-gold italic">Showrooms</span></h1>
          </div>
          <p className="text-white/40 text-sm max-w-xs mb-4">Descubre nuestros próximos eventos presenciales y virtuales diseñados para inversores globales.</p>
        </div>
        ...
        >
          <Image src={events[0].image} alt={events[0].title} fill className="object-cover opacity-60 group-hover:scale-105 transition-transform duration-1000" />
          ...
          <div className="absolute bottom-12 left-12 right-12 flex flex-col md:flex-row items-end justify-between gap-8">
            <div className="max-w-xl">
              ...
              <p className="text-white/60 font-light">{events[0].desc}</p>
            </div>
            <button className="px-10 py-4 bg-white text-[#0a101f] hover:bg-gold transition-colors text-xs font-bold uppercase tracking-widest shrink-0">Reservar Cupo</button>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-24">
          {events.slice(1).map((ev, idx) => (
            ...
            >
              ...
              <button className="text-gold text-xs font-bold uppercase tracking-widest flex items-center gap-2 hover:text-white transition-colors">Solicitar Agenda <ArrowRight className="w-4 h-4" /></button>
            </motion.div>
          ))}
        </div>
      </section>
